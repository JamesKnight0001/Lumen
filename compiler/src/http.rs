...
mod win {
    ...
    #[link(name = "winhttp")]
    extern "system" {
        fn WinHttpOpen(agent: *const u16, atype: u32, proxy: *const u16, byp: *const u16, fl: u32) -> H;
        fn WinHttpConnect(ses: H, host: *const u16, port: u16, res: u32) -> H;
        fn WinHttpOpenRequest(con: H, verb: *const u16, path: *const u16, ver: *const u16, refr: *const u16, acc: *const *const u16, fl: u32) -> H;
        fn WinHttpSendRequest(req: H, hdr: *const u16, hlen: u32, body: *const c_void, blen: u32, tot: u32, ctx: usize) -> i32;
        fn WinHttpReceiveResponse(req: H, res: *mut c_void) -> i32;
        fn WinHttpQueryHeaders(req: H, info: u32, name: *const u16, buf: *mut c_void, blen: *mut u32, idx: *mut u32) -> i32;
    ...
    struct UrlComp {
        size: u32,
        scheme: *mut u16, scheme_len: u32,
        n_scheme: i32,
        host: *mut u16, host_len: u32,
        port: u16,
        user: *mut u16, user_len: u32,
        pass: *mut u16, pass_len: u32,
        path: *mut u16, path_len: u32,
        extra: *mut u16, extra_len: u32,
    }
    ...
    pub fn get(url: &str) -> (i64, Vec<u8>) {
        unsafe {
            ...
            uc.host = host.as_mut_ptr(); uc.host_len = 255;
            uc.path = path.as_mut_ptr(); uc.path_len = 4095;
            if WinHttpCrackUrl(wurl.as_ptr(), 0, 0, &mut uc) == 0 {
                return (0, Vec::new());
            }
            let agent = wide("lumen-pkg/1.0");
            let ses = WinHttpOpen(agent.as_ptr(), AUTO_PROXY, std::ptr::null(), std::ptr::null(), 0);
            if ses.is_null() { return (0, Vec::new()); }
            let con = WinHttpConnect(ses, host.as_ptr(), uc.port, 0);
            let sec = if uc.n_scheme == SCHEME_HTTPS { FLAG_SECURE } else { 0 };
            let wverb = wide("GET");
            let req = WinHttpOpenRequest(con, wverb.as_ptr(), path.as_ptr(),
                std::ptr::null(), std::ptr::null(), std::ptr::null(), sec);
            let mut ok = WinHttpSendRequest(req, std::ptr::null(), 0, std::ptr::null(), 0, 0, 0) != 0;
            if ok { ok = WinHttpReceiveResponse(req, std::ptr::null_mut()) != 0; }
            let mut status: u32 = 0;
            let mut slen = 4u32;
            if ok {
                WinHttpQueryHeaders(req, Q_STATUS, std::ptr::null(),
                    &mut status as *mut u32 as *mut c_void, &mut slen, std::ptr::null_mut());
            }
